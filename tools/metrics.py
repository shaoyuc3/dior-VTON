...
import lpips
# from tools.PerceptualSimilarity.models import dist_model as dm
import pandas as pd
import json
import imageio, cv2
...
def resort(fns):
    ...
    pairLst = "/shared/rsaas/aiyucui2/inshop/fashion_yifang/fasion-pairs-test.csv"
    with open(pairLst) as f:
        anns = f.readlines()
        anns = [line[:-1].split(",")[1] for line in anns[1:]]
    #_, sort_idx = sorted(anns)
    sorted_idx = [i[0] for i in sorted(enumerate(anns), key=lambda x:x[1])]

    new_fns = [fns[i] for i in sorted_idx]
    return new_fns
...
class FID():
    ...
    def calculate_from_disk(self, imgs, gt_path):
        """ 
        """
        if not os.path.exists(gt_path):
            raise RuntimeError('Invalid path: %s' % gt_path)
        ...
        print('calculate gt_path statistics...')
        m1, s1 = self.compute_statistics_of_path(gt_path, self.verbose)
        # m1, s1 = self.compute_statistics_of_images(imgs_1, self.verbose)
        print('calculate generated_path statistics...')
        m2, s2 = self.compute_statistics_of_images(imgs, self.verbose)
        print('calculate frechet distance...')
        fid_value = self.calculate_frechet_distance(m1, s1, m2, s2)
        print('fid_distance %f' % (fid_value))
        return fid_value        

    def compute_statistics_of_images(self, imgs, verbose):
    ...
    def compute_statistics_of_path(self, path, verbose):
        ...
        if os.path.exists(npz_file):
            ...
            imgs = np.array([cv2.resize(imread(str(fn)).astype(np.float32), (256, 256)) for fn in files])
...
class LPIPS():
    def __init__(self, use_gpu=True):
        ...
        self.model = lpips.LPIPS(net='alex').cuda() # best forward scores
    ...
    def calculate_from_disk(self, path_1, path_2, batch_size=8, verbose=False, sort=True):
        #self, path_1, path_2, batch_size=64
        
        if sort:
            #files_1 = sorted(get_image_list(path_1))
            #files_2 = sorted(get_image_list(path_2))
            files_1 = resort(get_image_list(path_1))
            import pdb; pdb.set_trace()
            if path_2:
                files_2 = resort(get_image_list(path_2))
        else:
        ...
        # Bring images to shape (B, 3, H, W)
        '''
        imgs_1 = np.array(imgs_1).astype(np.float32)/127.5-1
        imgs_2 = np.array(imgs_2).astype(np.float32)/127.5-1
        '''
        ...
        w = W // 3
        if not path_2:
            imgs_1, imgs_2 = imgs_1[:,:,:,w*2:], imgs_1[:,:,:,w:w*2]
        else:
            imgs_1 = imgs_1[:,:,:,w*2:]
        result=[]
        ...
        for i in tqdm(range(n_batches), "lpips"):
            if verbose:
                print('\rPropagating batch %d/%d' % (i + 1, n_batches))
                      # end='', flush=True)
            start = i * batch_size
            ...
            result.append(self.model(img_1_batch, img_2_batch).detach().cpu())

        import pdb; pdb.set_trace()
        result = torch.cat(result).detach().cpu().numpy()
        distance = np.average(result)
        print('lpips: %.3f'%distance)
        return distance

    def calculate_mask_lpips(self, path_1, path_2, batch_size=16, verbose=False, sort=True):
        if sort:
            files_1 = sorted(get_image_list(path_1))
            files_2 = sorted(get_image_list(path_2))
        else:
...
def load_generated_images(images_folder, shrink_rate=1, sharp_rate=1, w=176):
    ...
    all_image_names = [fn for fn in os.listdir(images_folder)]# if fn[-4:] in [".jpg" or ".png"]]
    print("[loading] %d pairs of images" % len(all_image_names))
...
def load_generated_images_(images_folder, shrink_rate=1, sharp_rate=1):
    input_images = []
    target_images = []
    generated_images = []
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_path', help='Path to ground truth data', type=str)
    ...
    parser.add_argument('--sharp_rate', help='name of the experiment', type=int, default=1)
    ...
    args.crop_size = tuple(map(int, args.crop_size.split(', ')))
    if args.square: #args.crop_size >= 250:
        args.crop_size = (args.crop_size[0], args.crop_size[0])
    # load images
    sharp_rate = 1 #[1,2,3,4,5,6]
    all_fids, all_lpips = {},{}
    for shrink_rate in [1]: #range(100,50,-5):
        shrink_rate *= 0.01
        print("-----[curr_sharp_rate] %d--------" % sharp_rate)
        input_images, target_images, generated_images = load_generated_images(args.output_path, shrink_rate=shrink_rate, sharp_rate=sharp_rate, w=args.crop_size[1])
        #print('calculate fid metric...')
        #fid_score = fid.calculate_from_disk(generated_images, args.fid_real_path)
        #all_fids[shrink_rate] = fid_score
        #print('fid_score: ', fid_score)
        
        #gt_list, distorated_list = preprocess_path_for_deform_task(args.gt_path, args.output_path)
        ...
        print(input_images[0].shape, target_images[0].shape, generated_images[0].shape)
        print('calculate LPIPS...')
        #lpips_score = lpips.calculate_from_disk(target_images, generated_images, sort=False)
        lpips_score = lpips.calculate_from_disk(args.output_path, args.gt_path, sort=False)
        #lpips_score = lpips.calculate_from_disk(distorated_list, gt_list, sort=False)
        all_lpips[shrink_rate] = lpips_score
    ...
    print(all_lpips)
    #for r in all_fids:
        #print("[r=%.2f] fid:%.4f, lpips:%.4f" % (r, all_fids[r], all_lpips[r]))
