...
def torch_transform(img, flow_field):
        ...
        [b,_,h,w] = img.size()
        flow_field = torch.nn.functional.interpolate(flow_field, (h,w), mode='bilinear', align_corners=True)
        source_copy = img
        ...
        #warp = F.grid_sample(img, flow_field.permute(0, 2, 3, 1), mode='bilinear', padding_mode='border', align_corners=True)
