...
from models.networks.flowStyle.networks import VGGLoss, save_checkpoint
from models.networks.flowStyle.afwm import TVLoss, AFWM
...
opt.isDistributed = True

start_epoch, epoch_iter = 1, 0

dataset, dataset_sampler = create_dataset(opt)  # create a dataset given opt.dataset_mode and other options
...
if opt.continue_train:
    warp_model.load_state_dict(torch.load(opt.flownet_path), strict=False)
    print("load ckpt from %s and continue training."%opt.flownet_path)
    start_epoch = int(opt.flownet_path.split('.')[0][-3:])
...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for i, data in enumerate(dataset):
        ...
        #to_label = torch.unsqueeze(to_label, 1)
