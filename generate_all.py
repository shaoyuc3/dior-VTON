...
if __name__ == '__main__':
    ...
    generate_FID_dir = os.path.join(opt.eval_FID_dir + "_%s"%opt.epoch)
    print("generate images at %s" % generate_FID_dir)
    os.mkdir(generate_FID_dir)
    ...
    for i, data in tqdm(enumerate(dataset), "generating for test split"):  # inner loop within one epoch
        with torch.no_grad():
            ...
            model.forward()
            count = model.save_batch(generate_out_dir, generate_FID_dir, count, crop_size=opt.crop_size)
