...
from models.networks.flowStyle.afwm import TVLoss
...
class DIORBaseModel(BaseModel):
    ...
    def set_input(self, input):
        ...
        """
        from_img, from_kpt, from_parse, to_img, to_kpt, to_parse, to_dense, attr_label = input
        
        self.to_parse = to_parse.float().to(self.device)
        self.to_dense = to_dense.float().to(self.device)
    ...
    def save_visual(self, save_dir, count, square=False, crop_size=(256, 176)):
    ...
    def save_batch(self, save_dir, save_dir_FID, count, square=False, crop_size=(256, 176)):
        ...
        for i, ret in enumerate(rets):
            ...
            img_FID = tensor2im(self.fake_B[i])
            imageio.imwrite(os.path.join(save_dir_FID, "generated_FID_%d.jpg" % count), img_FID)
